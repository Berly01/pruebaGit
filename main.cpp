...
int main() {
	...
	//auto file{ FixedLengthFile<TitanicPerson>(FILE_TITANIC_PATH) };
...
void insert_records_b_tree(BPlusTreeFile<unsigned, unsigned, DEGREE>* _b_tree, FixedLengthFile<TitanicPerson>* _file) { //Crea un arbol b-tree ingresando los registros
	//almacenados no vacios de el archivo de longitud fija
	std::vector<TitanicPerson> vec_ti_person{};

	unsigned final_page{ _file->getTotalPages() };
	int id_passanget{};

	for (unsigned i = 0; i < final_page; i++) {
		...
		for (const auto& j : vec_ti_person) {
			...
			if (id_passanget != -1) {
				_b_tree->insert_key_value(id_passanget, i);
			}
		}
		vec_ti_person.clear();
	}
