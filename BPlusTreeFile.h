...
public:
	...
	void save_b_tree() {
		...
		//al arbol b_tree
		auto vec_keys{ b_tree.get_keys() };
		auto vec_values{ b_tree.get_vals() };

		const auto size_vec_keys{ vec_keys.size() };
		const auto size_vec_values{ vec_values.size() };

		output_file.write(reinterpret_cast<const char*>(&size_vec_keys), sizeof(size_t));
		output_file.write(reinterpret_cast<const char*>(&size_vec_values), sizeof(size_t));
